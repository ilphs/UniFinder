...
related: [unifinder-mvp-design, unifinder-followup-impl, unifinder-eject-diskspace-impl]
updated: 2026-08-20
---
...
### 5.1 여유 공간 표시 (2026-08-20)

```
┌──────────────────────────────────────────────────────────────┐
│ 128 items | 3 selected (14.2 MB)      Copying… 42%   196 GB free │
└──────────────────────────────────────────────────────────────┘
```

- **맨 오른쪽에 현재 폴더가 속한 볼륨의 여유 공간**을 `196 GB free`로 표시한다.
  진행 표시·안내 문구는 그 **왼쪽**에 온다 — 그 둘은 일시적이고 용량은 상시라, 순서를 바꾸면
  조작 중에만 숫자가 옆으로 밀려 눈이 따라가야 한다.
- 창이 좁아지면 **안내 문구가 먼저 잘린다**(용량은 `fixedSize`). 짧고 상시인 값이 줄어들면
  표시 자체가 의미를 잃는다.
- 값 조회 규칙은 디스크 용량 창과 **같은 `VolumeService`**를 쓴다(§7.7의 D1과 동일한 키·폴백).
  두 화면이 각자 계산하면 "창과 상태바의 숫자가 다르다"가 곧바로 버그로 보고된다.
- **조회 실패·볼륨 미특정이면 아무것도 그리지 않는다.** 용량 창은 표라서 행을 남기고 `--`를
  쓰지만, 상태바는 한 줄이라 `--`가 정보를 주지 않는다.
- **갱신 시점 (§7.7 D9의 명시적 예외)**: 용량 창은 "열 때 1회 + 수동 Refresh"지만, 상태바는
  항상 보이므로 같은 규칙이면 값이 영구히 낡는다(복사를 끝냈는데 여유 공간이 그대로면 그것이
  오답이다). 그래서:
  - 볼륨이 바뀌는 이동 → **즉시**
  - 같은 볼륨 안의 이동 → 마지막 조회로부터 **10초** 이후에만
  - `⌘R` · 파일 조작 완료 → **무조건**
  - **타이머 폴링 없음** — 아무 일도 없는 동안 디스크를 깨우는 비용이 표시 가치를 넘는다
- 조회(`.volumeURLKey` · 용량 키)는 전부 **메인 액터 밖**에서 한다(§5 성능 목표 "메인 블로킹 0").
...
Eject                                 (마운트된 볼륨을 우클릭했을 때만 — 항목 자체가 조건부)
...
| Eject | — | — | 우클릭한 항목이 **꺼낼 수 있는 볼륨 루트** && 선택 1개 (아니면 **항목 없음**) |
...
트리 노드 우클릭: Open(우측 표시), Open in New Window, [Eject], Copy, Paste, Rename,
Move to Trash, New Folder(하위 생성),
Add to Favorites / Remove from Favorites(2026-08-18 — 위험 대상 가드와 무관하게 항상 활성).
**트리 메뉴는 2026-08-19 개정에서 바뀌지 않았다** — 트리는 폴더만 다루므로 Open With가 의미 없고,
Get Info는 목록 선택을 대상으로 하는 항목이기 때문이다.

### 6.1 Eject (2026-08-20)

- **대상이 아닐 때는 비활성이 아니라 항목 자체를 넣지 않는다.** 이 메뉴의 다른 항목들은
  "선택에 따라 비활성"이 자연스럽지만, 트리·목록의 대다수 항목은 일반 폴더/파일이라 항상 넣으면
  죽은 Eject가 상시로 보인다. Finder도 볼륨에서만 보여준다.
- **부팅 볼륨은 절대 대상이 아니다.** 판정은 `volumeIsRootFileSystem`이 정본이고, 키가 없으면
  경로가 `/`인지로 폴백한다 — 외장 디스크로 부팅한 머신에서는 루트 볼륨이 `isEjectable == true`로
  보고되므로 경로만 보면 "부팅 디스크 꺼내기"를 권하게 된다.
- 적격 판정 근거는 **볼륨 열거 시점에 캐시한 속성**이다. 우클릭 시점에 볼륨을 다시 조회하면
  응답 없는 마운트가 하나 끼는 순간 컨텍스트 메뉴 자체가 멈춘다.
- **다중 선택에는 넣지 않는다**(`Rename`·`Get Info`와 같은 단일 선택 규칙) — 여러 볼륨을 한 번에
  꺼내면 실패가 섞였을 때 무엇이 남았는지 알릴 방법이 없다.
- 성공 시 앱은 **아무것도 하지 않는다**: 언마운트 통지가 트리 갱신과 "그 볼륨 안에 있었으면
  홈으로 이동"을 이미 처리한다(m3-impl T1). 실패는 모달이 아니라 **상태바 문구**로 알린다(§9).
- 단축키는 주지 않는다(Finder도 없다).
